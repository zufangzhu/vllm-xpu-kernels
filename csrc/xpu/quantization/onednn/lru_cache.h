...
struct lru_cache_standalone
 ...
 public:
  ...
  inline bool find_key(key_t key) { return cache::find(key) != cache::end(); }
  ...
  inline value_t fetch_m(key_t key) { return cache::fetch(cache::find(key)); }
};
